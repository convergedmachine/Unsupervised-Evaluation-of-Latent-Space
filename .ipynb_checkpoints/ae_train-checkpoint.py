...
def preprocess(trial, X: np.ndarray) -> PrepResult:
    """Only column-normalization with numeric stability and thresholding."""
    ...
    info = {'thresh': float(thresh), 'kept': int(mask.sum())}
    return PrepResult(X=Xp, preproc='colnorm', info=info)
...
class Autoencoder7LayerCL(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h1 = self.act(self.fc1(x)); h1 = self.cl1(h1)
        h2 = self.act(self.fc2(h1)); h2 = self.cl2(h2)
        h3 = self.act(self.fc3(h2)); h3 = self.cl3(h3)
        z  = self.act(self.fc4(h3)); z  = self.cl4(z)
        d3 = self.act(self.fc5(z));  d3 = self.cl5(d3)
        d2 = self.act(self.fc6(d3)); d2 = self.cl6(d2)
        d1 = self.act(self.fc7(d2)); d1 = self.cl7(d1)
        xhat = self.fc_out(d1)
        return xhat
...
def objective_ae(trial, X: np.ndarray, n_splits:int=5, max_epochs:int=200, device:str='cpu'):
    ...
    h1 = trial.suggest_int('nhid1', 32, 2048, step=32)
    h2 = trial.suggest_int('nhid2', 32, 2048, step=32)
    h3 = trial.suggest_int('nhid3', 16, 2048, step=16)
    z  = trial.suggest_int('ncode',  8,  1024, step=8)
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(kf.split(Xp)):
        ...
        for name in ed_lists.keys():
            ...
            ed_lists[name].append(energy_distance_to_gaussian_from_C(W))
...
def main():
    ...
    ap.add_argument("--cv", type=int, default=5, help="K-folds")
