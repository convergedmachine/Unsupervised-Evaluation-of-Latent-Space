...
class AMat:
    ...
    def __init__(self, path, head=None):
        ...
        nshape = (len(data) // len_float_line, len_float_line)  # Integer division
...
class BaseL2007(object):
    ...
    def main_show(cls):
        ...
        def view_images(img_array, labels=None):
            for i in range(len(img_array)):
                plt.imshow(img_array[i].reshape(28, 28), cmap='gray')
                if labels is not None:
                    plt.title(f"Label: {labels[i]}")
                plt.axis('off')
                plt.show()        
        view_images(img_array=self._inputs, labels=labels)
