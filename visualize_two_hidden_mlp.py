...
# ---------------- I/O ----------------
def load_histories_twohidden(root="two_hidden_mlp_records/histories", dataset="cifar100"):
    """
...
def plot_grid(dataset: str,
    ...
    for idx, (h1, h2) in enumerate(all_h1h2):
        ...
        if best_acc is not None and np.isfinite(best_acc):
            ax.text(0.98, 0.02, f"Best: {best_acc:.3f}",
...
if __name__ == "__main__":
    datasets = ["cifar100"]
    for d in datasets:
        hists = load_histories_twohidden("two_hidden_mlp_records/histories", dataset=d)
        if not hists:
