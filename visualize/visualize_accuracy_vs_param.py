...
import glob
import pandas as pd
from vizs_helpers import plot_accuracy_vs_param_panels
...
csv_files = glob.glob("../records/one_hidden_mlp_records/*/*trials.csv")
...
plot_accuracy_vs_param_panels(parzen_dict, random_dict,
                       ...
                       save_path="acc_vs_param_1x8_panels_scatter_1l.png"
                     )

csv_files = glob.glob("../records/three_hidden_mlp_records/*/*trials.csv")
...
plot_accuracy_vs_param_panels(parzen_dict, random_dict,
                       acc_col="value", param_col="avg_fit_criteria",
                       save_path="acc_vs_param_1x8_panels_scatter_3l.png"
                     )

csv_files = glob.glob("../records/ae_records/*/*trials.csv")

# Create dictionaries with specific suffix removal
parzen_files = [f for f in csv_files if os.path.basename(os.path.dirname(f)).endswith("_parzen")]
random_files = [f for f in csv_files if os.path.basename(os.path.dirname(f)).endswith("_random")]
parzen_dict = make_dict(parzen_files, "_parzen", "ae_")
random_dict = make_dict(random_files, "_random", "ae_")

plot_accuracy_vs_param_panels(parzen_dict, random_dict,
                       acc_col="user_attrs_val_recon_loss_mean", param_col="user_attrs_ed_overall_mean",
                       save_path="acc_vs_param_1x8_panels_scatter_ae.png"
                     )